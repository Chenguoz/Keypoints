...
if __name__ == '__main__':
    ...
    split = 'train'
    segmentation = True
    d = Dataset(root='../../', dataset_name=dataset_name, class_choice='motorbike', num_points=2048, split=split, segmentation=segmentation)
    print("datasize:", d.__len__())

    item = 1
    if segmentation:
        ps, lb, seg, n, f = d[item]
        print(ps.size(), ps.type(), seg.size(), seg.type, lb.size(), lb.type(), n, f)
        print(seg)
    else:
        ps, lb, n, f = d[item]
