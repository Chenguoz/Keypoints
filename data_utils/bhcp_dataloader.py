...
class bhcp_dataloader(data.Dataset):
    def __init__(self, data_root, category, is_pts_aligned=False, preload_data=True, split='train'):
        super().__init__()
        if split == 'train':
        # self.data_path = os.path.join(data_root , category)

            self.data_path = os.path.join(data_root + '/training_data', category)
        else:
            self.data_path = os.path.join(data_root + '/test_data', category)
    ...
    def __getitem__(self, idx):
        ...
        res['data_id'] = idx
        return points,meta_data['feat_pts']
        # return res
